  ...
  Future<void> _uploadAndParseReceipt() async {
    ...
    if (pickedFile != null) {
      ...
      double? _extractAmount(String text) {
        // Accept ₹, Rs, INR, Z, R5, R$, RS, etc.
        final regex = RegExp(
          r'(?:₹|Rs\.?|INR|Z|R5|R\$|RS)[\s:]*([\d,]+(?:\.\d{1,2})?)',
          caseSensitive: false,
        );
        final match = regex.firstMatch(text);
        if (match != null) {
          String rawAmount = match.group(1)!;
          rawAmount = rawAmount.replaceAll(',', '').trim();
      ...
      double? amount;
// Step 1: Try with ₹ / Rs / INR regex
      for (final line in lines) {
        amount = _extractAmount(line);
        if (amount != null) break;
      }

// Step 2: If still null, check for standalone numbers intelligently
      if (amount == null) {
        double? candidate;
        for (int i = 0; i < lines.length; i++) {
          final line = lines[i];
          if (RegExp(r'^\d+(\.\d{1,2})?$').hasMatch(line)) {
            final numVal = double.tryParse(line);

            // Check surrounding context words
            final context = (i > 0 ? lines[i - 1] : '') +
                ' ' +
                (i < lines.length - 1 ? lines[i + 1] : '');
            if (context.toLowerCase().contains('paid') ||
                context.toLowerCase().contains('debited') ||
                context.toLowerCase().contains('credited') ||
                context.toLowerCase().contains('successful') ||
                context.toLowerCase().contains('transaction')) {
              candidate = numVal;
              break;
            }

            // If nothing else, keep track of the largest number
            if (candidate == null || (numVal != null && numVal > candidate)) {
              candidate = numVal;
            }
          }
        }
